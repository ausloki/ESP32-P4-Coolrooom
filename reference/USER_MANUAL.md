...
| **Evaporator Probe (Probe 2) Enabled** | Master switch for the evaporator RTD (same 2CH PT100 as Probe 1, channel 2). | On/Off | **On** | Leave **On** on this plant — the coil probe is fitted. Off hides the Evap readout and disables Smart Defrost (§4.3) and the Ice Alarm (§4.5), even if those switches are on. Does **not** reassign which physical sensor is Probe 1 or Probe 2. |
